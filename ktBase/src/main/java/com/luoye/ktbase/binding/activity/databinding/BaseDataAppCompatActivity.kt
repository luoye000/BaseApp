 ...
 *
 */
abstract class BaseDataAppCompatActivity<V : ViewDataBinding, M : ViewModel> : AppCompatActivity() {

    protected var TAG = "---"
    protected val context: Context by lazy(mode = LazyThreadSafetyMode.NONE) { this }
        @Deprecated
        get
    protected val binding by lazy(mode = LazyThreadSafetyMode.NONE) {
        getViewBinding<V>(inflater = layoutInflater, position = 0)
    }
    protected val viewModel by lazy(mode = LazyThreadSafetyMode.NONE) {
        createViewModel<M>(this, 0)
    }
